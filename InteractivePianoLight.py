...
from library.ledsettings import LedSettings
from library.ledstrip import LedStrip
from library.menulcd import MenuLCD
from library.midiports import MidiPorts
from library.savemidi import SaveMIDI
...
if not args.skipupdate:
    ...
    if not os.path.exists('/usr/local/bin/connectall.py') or \
        filecmp.cmp('/usr/local/bin/connectall.py', 'library/connectall.py') is not True:
        print("connectall.py script is outdated, updating...")
        copyfile('library/connectall.py', '/usr/local/bin/connectall.py')
        os.chmod('/usr/local/bin/connectall.py', 493)
        

#Rotate the screen and also the key
            ...
            if saving.isrecording:
                if ledsettings.color_mode == "Multicolor":
                    saving.add_track("note_on", original_note, velocity, midiports.last_activity,
                                     webcolors.rgb_to_hex((red, green, blue)))
                else:
                    saving.add_track("note_on", original_note, velocity, midiports.last_activity)
        else:
            control = find_between(str(msg), "control=", " ")
